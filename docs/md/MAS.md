...
- 目标：解决多个自治的 Agent 在同一个环境下的序贯决策问题
- 类型：完全合作、完全竞争、混合型(组内合作，组间竞争)

符号：暂略

策略网络：用神经网络近似策略函数

常见的实验环境：多 Agent 粒子世界 / 星际争霸2

###### 完全合作设定下的 MADRL

**假设** 所有的局部观察的总和构成全局状态：$S=[O^1,O^2,\cdots,O^m]$

训练和执行时的困难：

- 在训练时，值/策略网络需要知道全局状态 s 才能计算 TD 误差和梯度
- 在执行时，Agent 不能独立做决策，因为它的决策需要全局状态 s
...
- CTCE 集中训练/集中执行 值网络输入为 s / 策略网络输入为 s 

- DTDE 分布训练/分布执行 值网络输入为 o / 策略网络输入为 o 

- DTCE 值网络输入为 s / 策略网络输入为 o 
...
- MADDPG 针对连续控制问题，MAN-A2C 针对离散控制问题
- MADDPG 是异策略(off-policy)，MAN-A2C 是同策略(on-policy)
